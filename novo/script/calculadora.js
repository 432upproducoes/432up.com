...
function animateValue(start, end, duration, elements) {
  ...
  function updateNumber(currentTime) {
    ...
    // Curva de desaceleração suave (easeOutCubic)
...
function calculateAtmosphere() {
  ...
  // 1. REGRA DO STICKY: Se não tiver nada selecionado, zera e oculta
  if (!hasAnySelection) {
  ...
  // Controle de visibilidade da barra (Exclusivo Mobile e caso o preço não esteja em tela)
  ...
  // 3. COMPORTAMENTO DINÂMICO DO BOTÃO DA BARRA MOBILE
  if (stickyBtnWhatsapp) {
    if (missingSteps.length > 0) {
      // PROJETO INCOMPLETO: Remove o estado pronto e aplica estilo de transição (Ametista)
      ...
      stickyBtnWhatsapp.onclick = (e) => {
        ...
        // Identifica qual é o primeiro campo faltante e conduz com elegância
        let target = null;
        let mensagemEducada = "";
        ...
        if (target) {
          ...
          // Opcional: Destaque sutil ou foco na mesma linguagem acolhedora da agência
          console.info("432UP! Condução: " + mensagemEducada);
        }
      };
    } else {
      // PROJETO COMPLETO: Aciona a classe CSS pronta para o Verde Neon (.ready-state)
...
// 4. PREPARA A MENSAGEM DE ORIENTAÇÃO (só prepara, não aplica ainda)
  let guidanceMessage = "";
  if (missingSteps.length > 0) {
  ...
  // 5. CÁLCULO RIGOROSO DO INVESTIMENTO
  let basePrice = 0;
  ...
  // 6. EXIBIÇÃO FINAL – decide UMA ÚNICA VEZ com base no valor
  if (finalTotal > 0) {
    // Tem valor → mostra o preço (mesmo que ainda falte algo)
    ...
    // Não tem valor → mostra a frase de orientação
    if (priceEl) {
...
function setupStickyScrollObserver() {
  ...
  const observer = new IntersectionObserver((entries) => {
    ...
    threshold: 0.2 // Oculta a barra quando 20% do bloco do preço estiver visível na tela
  });
